...
class MagneT(object):
    ...
    def gEgaussian(self, B=None, E=None, Gam=None, Nmax=None):
        ...
        if B:
            self._B = B
        if isinstance(E, (np.ndarray, float, int)):
            self._E = E
        if Gam:
            self._Gam = Gam
        if Nmax:
        ...
        n = arange(self._N + 1)[:, newaxis, newaxis]  # N,1 elements (N=Nmax+1)
        En = k.hbar * wc * (n + 1.0 / 2)  # NxM elements)
        self._dos = (
            1.0
            / (pi * l ** 2)
            * 1
            / (sqrt(2 * pi) * self._Gam)
            * sum(exp(-((self._E - En) ** 2) / (2 * self._Gam ** 2)), axis=0)
        )
        return self._dos
    ...
    def gElorentzian(self, B=None, E=None, Gam=None, Nmax=None):
        ...
        self._dos = (
            1.0
            / (pi * l ** 2)
            * sum(self._Gam / ((self._E - En) ** 2 + self._Gam ** 2), axis=0)
        )
        return self._dos
    ...
    def Dbe(self, B=None, E=None, Gam=None, Xi=None, Nmax=None, gE=gEgaussian):
        ...
        if not isinstance(B, (list, ndarray)) and not isinstance(E, (list, ndarray)):
            ...
            Ggi = self.gE()
            self._dos = (
                self._Xi * m / (pi * k.hbar ** 2)
                + (1 - self._Xi) * 2 * k.e * B / (pi * k.h) * Ggi
            )
            if single:
                return self._dos[0]
            return self._dos
    ...
    def gESS(
        ...
        if B:
            self._B = B
        if isinstance(Bs, (np.ndarray, float, int)):
            self._Bs = Bs
        if Gam:
            self._Gam = Gam
        if Nmax:
        ...
        En = k.hbar * wc * (n + 1.0 / 2)
        EnP = k.hbar * wc * (n + 1.0 / 2) + Dp + ep
        EnN = k.hbar * wc * (n + 1.0 / 2) - Dn
        if GL == 1:
            if self._Bs != 0:
                self._dos = self._Xi * self._m / (pi * k.hbar ** 2) + (
                    ...
                    (
                        ...
                        * 1
                        / (sqrt(2 * pi) * self._Gam)
                        * sum(
                            exp(-((self._E - EnP) ** 2) / (2 * self._Gam ** 2)), axis=0
                        )
                    )
                    + (
                        1.0
                        ...
                        * 1
                        / (sqrt(2 * pi) * self._Gam)
                        * sum(
                            exp(-((self._E - EnN) ** 2) / (2 * self._Gam ** 2)), axis=0
                        )
                    )
                )
            else:
                self._dos = self._Xi * self._m / (pi * k.hbar ** 2) + (
                    1 - self._Xi
                ) * 1.0 / (pi * l ** 2) * 1 / (sqrt(2 * pi) * self._Gam) * sum(
                    exp(-((self._E - En) ** 2) / (2 * self._Gam ** 2)), axis=0
                )
        else:
            if self._Bs != 0:
                self._dos = self._Xi * self._m / (pi * k.hbar ** 2) + (
                    ...
                    sum(self._Gam / ((self._E - EnP) ** 2 + self._Gam ** 2), axis=0)
                ...
                self._dos = self._Xi * self._m / (pi * k.hbar ** 2) + (
                    ...
                    self._Gam / ((self._E - En) ** 2 + self._Gam ** 2), axis=0
                )
        return self._dos
    ...
    def fermi_weight(self, E=None, EF=None, T=None):
        ...
        ret = log(1.0 - 1.0 / (1 + exp(a)))
        return ret

    def Omega(
        self, B=None, EF=None, T=None, Gam=None, Xi=None, Nmax=None, gE=gEgaussian
    ):
        """
    ...
    def OmegaA(
        self, B=None, ns=None, T=None, Gam=None, Xi=None, m=None, Nmax=None, GL=None
    ):
        """
        ...
        I4 = -((hwc) ** 2) / (4 * pi ** 2 * n ** 2 * k.k * self._T) + hwc / (
            2 * n
        ) * cos(2 * pi * n * self._mu / hwc) / sinh(
            2 * pi ** 2 * n * k.k * self._T / hwc
        )
        if self._GL == 0:
            self._Om = (
                self._m
                ...
                * (
                    ...
                    * (1 - self._Xi)
                    * sum(
                        (-1) ** n * exp(-2 * (n * pi * self._Gam) / (hwc)) * I4, axis=0
                    )
                )
            )
        else:
    ...
    def MagA(
        ...
        hwc = k.hbar * wc
        smu = (
            2 * pi * n * self._mu / (hwc) + phi * 2 * pi * n
        )  # 2nd term is a Berry phase
        skt = 2 * pi ** 2 * n * k.k * self._T / (hwc)
        Gam = self._Gam * self._B ** self._p
        I4 = -((hwc) ** 2) / (4 * pi ** 2 * n ** 2 * k.k * self._T) + hwc / (
            2 * n
        ) * cos(smu) / sinh(2 * pi ** 2 * n * k.k * self._T / hwc)
        DI4 = (
            1
            / self._B
            * (
                -(hwc ** 2) / (2 * pi ** 2 * n ** 2 * k.k * self._T)
                + pi * self._mu * sin(smu) / sinh(skt)
                + cos(smu)
                / sinh(skt)
        ...
        if self._GL == 1:
            self._Mag = (
                ...
                * sum(
                    (-1) ** n
                    * exp(-2 * (n * pi * Gam) ** 2 / (hwc) ** 2)
                    * (
                        DI4
                        + (1 - self._p) * (2 * pi * n * Gam / hwc) ** 2 * I4 / self._B
                    ),
                    axis=0,
                )
            )
            ...
            self._Mag = (
                ...
                * sum(
                    (-1) ** n
                    * exp(-2 * (n * pi * Gam) / (hwc))
                    * (DI4 + (1 - self._p) * (2 * pi * n * Gam / hwc) * I4 / self._B),
                    axis=0,
                )
            )
        return self._Mag
    ...
    def nsb(
        ...
        return (
            ...
            * (
                ...
                * (1 - self._Xi)
                * sum(
                    (-1) ** n * exp(-2 * (n * pi * self._Gam) ** 2 / (hwc) ** 2) * I2,
                    axis=0,
                )
            )
        )
    ...
    def nsbL(
        ...
        return (
            ...
            * (
                ...
                + 2
                * (1 - self._Xi)
                * sum((-1) ** n * exp(-2 * (n * pi * self._Gam) / (hwc)) * I2, axis=0)
            )
        )
